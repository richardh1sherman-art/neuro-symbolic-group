...
class PerfectlyAlignedHanoiSystem:
    def __init__(self, num_disks):
        ...
        # --- CORRECTED BIPARTITE TRANSITION DICTIONARY ---
        disk_powers = torch.arange(self.num_disks, device=device)
        
        # FIX: Flip the bit extraction order to align binary bit significance
        flipped_powers = torch.flip(disk_powers, dims=[0])
        bits = (base_indices.unsqueeze(1) >> flipped_powers) & 1
        
        peg_placements = bits * 2
        
        # FIX: Keep strides aligned with the newly flipped bit ordering
...
# Initialize an input state at index 2 (Binary 010)
init_state_m2 = torch.zeros(system_val.m2_dim, 1, device=device)
init_state_m2[2] = 1.0 

# Run a deterministic step (Index 2 moves to Index 3 -> Binary 011)
output_state_m2 = system_val.run_deterministic_gen1_step(init_state_m2)

# Pass through the corrected bipartite dictionary translation
...
# Method 1 native lookup for index 18 (Ternary equivalent to the targeted state configuration)
